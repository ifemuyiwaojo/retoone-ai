...
export default function Player() {
  ...
  const { addToHistory, getNextTrack } = useQueue();
  ...
  return (
    <>
      {isExpanded && (
        <div 
          className="fixed inset-0 ml-64 bg-black/50 backdrop-blur-sm z-40"
          onClick={() => setIsExpanded(false)}
        />
      )}
      
      <div className={`fixed bottom-0 left-64 right-0 bg-gradient-to-br from-gray-900 via-gray-800 to-black border-t border-white/5 shadow-2xl transition-all duration-300 ease-in-out z-50 ${
        isExpanded ? 'h-[calc(100vh-64px)]' : 'h-24'
      }`}>
